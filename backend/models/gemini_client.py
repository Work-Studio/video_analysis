        ...
        try:
            payload_json = await self._invoke_gemini(
                ...
                instruction=(
                    ...
                    "各テキストについて、動画内で表示されたタイムコード（MM:SS.D形式、0.1秒精度、例: 1:23.5）を含めてください。"
                    "JSON形式で以下の構造で出力してください:\n"
                    "{\n"
                    '  "texts": [\n'
                    "    {\n"
                    '      "timecode_start": "MM:SS.D",\n'
                    '      "timecode_end": "MM:SS.D",\n'
                    '      "text": "表示されたテキスト"\n'
                    "    }\n"
                    "  ]\n"
                    "}\n\n"
        ...
        try:
            payload_json = await self._invoke_gemini(
                ...
                instruction=(
                    ...
                    "各発話について、タイムコード（MM:SS.D形式、0.1秒精度）を含めてください。"
                    "読点（、）句点（。）疑問符（？）感嘆符（！）で文章を区切り、それぞれにタイムコードを付与してください。"
                    "JSON形式で以下の構造で出力してください:\n"
                    "{\n"
                    '  "segments": [\n'
                    "    {\n"
